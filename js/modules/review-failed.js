...
(function () {
    ...
    async function initReviewFailPage() {
        const { caseId } = parseHash(location.hash);

        if (!caseId) {
            alert('找不到案件編號');
            window.location.href = '../html/member-center.html'; //會員中心
            return;
        }
        ...
        if (!data.rejected) {
            if (data.rejected) {
                location.hash = `#review-success?caseId=${caseId}`;
            } else {
                location.hash = `#adoption-review?caseId=${caseId}`;
            }
            return;
        }
        ...
        if (caseNumberEl) {
            caseNumberEl.textContent = `案件編號：${data.caseNumber}`;
        }
    }
